...
class YahooFinanceFundamentalService(
    ...
    suspend fun fetchAndSaveFundamentalData(symbol: String): Boolean {
        return try {
            val fundamentalData = fetchFundamentalData(symbol)

            // Save company overview
            fundamentalData.companyOverview?.let {
                fundamentalAnalysisService.saveCompanyOverview(it)
            }

            // Save income statements
            fundamentalData.incomeStatements.forEach { statement ->
                fundamentalAnalysisService.saveIncomeStatement(statement)
            }

            // Save balance sheets
            fundamentalData.balanceSheets.forEach { sheet ->
                fundamentalAnalysisService.saveBalanceSheet(sheet)
            }

            // Save cash flow statements
            fundamentalData.cashFlowStatements.forEach { statement ->
                fundamentalAnalysisService.saveCashFlowStatement(statement)
            }

            true
        } catch (e: Exception) {
            println("Error fetching fundamental data for $symbol: ${e.message}")
            false
        }
    }
